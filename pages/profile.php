...
<div class="min-h-screen bg-[url(<?= $base_url ?>/static/images/bg.jpg)] bg-center bg-cover bg-no-repeat">
    ...
    <div class="relative min-h-screen flex flex-col">
        <header class="flex h-16 bg-white p-5 justify-between items-center border-b-[1px] border-b-amber-100">
            ...
            <a href="main" class="hover:text-amber-400">Главная</a>
            <div class="flex items-center">
                ...
                />
                <a href="profile" class="hover:text-amber-400"><?= ($_SESSION['user']['firstname'] ?? '') . ' ' . ($_SESSION['user']['lastname'] ?? '') ?></a>
            </div>   
        </header>
        
        <main class="flex-1 overflow-auto">
            <?php if (isset($_SESSION['user'])): ?>
                <form onsubmit="validateFile(event, 'avatar')" method="POST" action="api/avatar/update" enctype="multipart/form-data" class="flex flex-col gap-4 w-96 p-6 bg-white mx-auto mt-20">
                    <h2 class="font-bold text-center text-xl">Смена аватара</h2>
                ...
                <form class="flex flex-col inset-0 m-auto w-80 h-96 mt-20 bg-white border-[1px] border-amber-200" method="post" action="api/password/change">
                    ...
                    <div class="flex flex-col m-6 grow justify-between items-center">
                        <input name="csrf_token" value="<?= $_SESSION['csrf_token'] ?>" type="text" hidden readonly/>
