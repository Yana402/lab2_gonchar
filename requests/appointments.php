...
if ($_SESSION['user']['role'] == 'c')
    $stmt = $connection->prepare("SELECT appointments.id, date_time, firstname, lastname, speciality, status FROM appointments INNER JOIN masters ON master_id = masters.user_id INNER JOIN users ON master_id = users.id WHERE appointments.user_id = ? ORDER BY date_time");
